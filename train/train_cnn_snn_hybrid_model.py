...
# TODO check if initialization is done correctly 
...
if __name__ == '__main__':
    ...
    if save_runs: 
        ...
        comment = '_HYBRID_CNN_SNN_DHP19_mp_loss_ratio_{}'.format(mp_loss_ratio) + flags.tb_name
        tb = SummaryWriter(comment=comment)

    step_counter = 0

    num_bins = training_config.getint('num_bins')
    loss_states = nn.MSELoss()
    for epoch in range(training_config.getint('num_epochs')):
        ...
        for batch_id, sample_batched in enumerate(train_loader):
            ...
            for i in range(num_bins):
                if i == 0: 
                    output, cnn_output = model(inputs[:,i].float(), inputs_cnn[:,0].float(), v_init=True) # todo
                else:
                    output, mp_vals_snn = model(inputs[:,i].float(), v_init=False)

                out_loss += my_mse_loss(output, labels[:, i].float(), sigma=2, device=device)/num_bins
            ...
            # get new states
            mp_vals_cnn = model(x_snn = None, x_cnn=inputs_cnn[:,1].float(), v_init=True) # todo

            mp_loss = 0
            for layer_name, val in mp_vals_cnn.items():
                mp_loss += loss_states(mp_vals_cnn[layer_name], mp_vals_snn[layer_name])/9

            # last sample
            mpjpe = mpjpe_temp
            loss = (1-mp_loss_ratio)*out_loss + mp_loss_ratio*mp_loss
        ...
        with torch.no_grad(): # computational graphs are not created
            ...
            val_mpjpe_list = np.zeros(val_num_bins)
            ...
            for val_batch_id, val_sample_batched in enumerate(val_loader):
                ...
                val_loss = 0
                val_mpjpe_avg10 = 0
                for i in range(val_num_bins):

                    if i == 0: 
                        val_output, val_cnn_output = model(val_inputs[:, i].float(), val_inputs_cnn.float(), v_init=True) # todo
                    else:
                        val_output, _ = model(val_inputs[:, i].float(), v_init=False)

                    val_loss += my_mse_loss(val_output, val_labels[:, i].float(), sigma=2, device=device)/val_num_bins
                    val_pred_landmarks = get_pred_landmarks(val_output)
                    val_mpjpe_temp = calc_mpjpe(val_pred_landmarks, val_gt_landmarks[:, :, i]).item()
                    val_mpjpe_avg10 += val_mpjpe_temp/val_num_bins
            ...
            if save_runs:
                ...
                fig, ax = plt.subplots()
                ax.plot(val_mpjpe_list/val_tot_batch_size)
                ax.set_xlabel('Bin number')
                ax.set_ylabel('MPJPE')
                img = plot_to_tensor()
