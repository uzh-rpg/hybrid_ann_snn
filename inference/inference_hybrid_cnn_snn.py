...
if __name__ == '__main__':
    ...
    ablation_mpjpe = np.zeros((num_bins))
    ...
    with torch.no_grad(): # computational graphs are not created
        for val_batch_id, val_sample_batched in tqdm(enumerate(val_loader), total=val_loader.__len__(), desc='Total batches: '):
            ...
            for i in range(num_bins):
                if i == 0: 
                    val_output, val_cnn_output, mid_res = model(val_inputs[:,i].float(), val_inputs_cnn.float(), v_init = True)
                    cnn_lanmarks = get_pred_landmarks(val_cnn_output)
                else:
                    val_output, mid_res = model(val_inputs[:,i].float(), v_init = False)

                ablation_mpjpe[i] += calc_mpjpe(cnn_lanmarks, val_gt_landmarks[:, :, i])*val_sample_batch_size
    ...
    with open(flags.txt_path, 'w') as f: 
        ...
        f.write('%s:%s\n' % ('ablation_mpjpe_list', ablation_mpjpe/val_tot_batch_size))
