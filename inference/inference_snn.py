...
if __name__ == '__main__':
    ...
    model = EVSNN_LIF_final(kwargs=model_params_config, inference=True).to(device)
    ...
    get_spikes = manage_hooks(model, hybrid = False, all_snn = True)
    ...
    with torch.no_grad(): # computational graphs are not created
        for val_batch_id, val_sample_batched in tqdm(enumerate(val_loader), total=val_loader.__len__(), desc='Total batches: '):
            ...
            for i in range(num_bins):
                val_output, mid_res = model(val_inputs[:,i].float(), prev_mem_states=None)
