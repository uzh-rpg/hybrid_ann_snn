    ...
    with torch.no_grad(): # computational graphs are not created
        for val_batch_id, val_sample_batched in tqdm(enumerate(val_loader), total=val_loader.__len__(), desc='Total batches: '):
            ...
            for i in range(num_bins):
                if i == 0:
                    val_output, val_states, _ = model(val_inputs[:, i].float(), val_inputs_cnn.float(), v_init=True, prev_states=val_prev_states) # todo
                else:
                    val_output, val_states = model(val_inputs[:, i].float(), v_init=False, prev_states=val_prev_states)
