...
class Spiking_residualBlock(nn.Module):
    ...
    def forward(self, x, v_init):
        ...
        out = self.bn1(out)
        out = self.activation1(out, v_init[0])
        mid_res = out
        ...
        out += residual
        out = self.activation2(out, v_init[1])
        return out, mid_res
...
class IF_Layer(nn.Module):
    def __init__(self, surrogate_function, detach_reset, v_threshold=1.0, v_reset=None):
...
class Hybrid_CNN_SNN(BasicModel):
    ...
    def forward(self, x_snn, x_cnn=None, v_init=False): # 10 layers
        ...
        x_in_snn = self.static_conv_snn(x_snn, v_init=v_init_in)

        x1_snn = self.down1_snn(x_in_snn, v_init=v_init_1)
        x2_snn = self.down2_snn(x1_snn, v_init=v_init_2)
        x3_snn = self.down3_snn(x2_snn, v_init=v_init_3)

        r1_snn, mid_res = self.residualBlock_snn(x3_snn, v_init = v_init_r) # 2 conv layers

        u1_snn = self.up1_snn(torch.cat([r1_snn, x3_snn], 1), v_init=v_init_6)
        u2_snn = self.up2_snn(torch.cat([u1_snn, x2_snn], 1), v_init=v_init_7)
        u3_snn = self.up3_snn(torch.cat([u2_snn, x1_snn], 1), v_init=v_init_8)
