...
from .rnn_submodules import ConvLayer, UpsampleConvLayer, TransposedConvLayer, RecurrentConvLayer, ResidualBlock, ConvLSTM, ConvGRU
# from timers import *
import pdb
...
class BaseUNet(nn.Module):
    def __init__(self, num_input_channels, num_output_channels=1, skip_type='sum', activation='sigmoid',
                 num_encoders=4, base_num_channels=32, num_residual_blocks=2, norm=None, running_stats=True, use_upsample_conv=True):
        ...
        #     self.activation = getattr(torch, self.activation, 'relu')
        # elif activation == 'tanh':
        #     self.activation = getattr(torch, self.activation, 'tanh')
    ...
    def build_resblocks(self):
        ...
        for i in range(self.num_residual_blocks):
            self.resblocks.append(ResidualBlock(self.max_num_channels, self.max_num_channels, norm=self.norm, running_stats=self.running_stats))
    ...
    def build_decoders(self):
        ...
        for input_size in decoder_input_sizes:
            self.decoders.append(self.UpsampleLayer(input_size if self.skip_type == 'sum' else 2 * input_size,
                                                    input_size // 2,
                                                    kernel_size=5, padding=2, norm=self.norm, running_stats=self.running_stats))

    def build_prediction_layer(self):
        self.pred = ConvLayer(self.base_num_channels if self.skip_type == 'sum' else 2 * self.base_num_channels,
                              self.num_output_channels, 1, activation=None, norm=self.norm, running_stats=self.running_stats)
...
class UNet(BaseUNet):
    def __init__(self, num_input_channels, num_output_channels=1, skip_type='sum', activation='sigmoid',
        ...
        super(UNet, self).__init__(num_input_channels, num_output_channels, skip_type, activation,
                                   num_encoders, base_num_channels, num_residual_blocks, norm, running_stats, use_upsample_conv)
        ...
        for input_size, output_size in zip(self.encoder_input_sizes, self.encoder_output_sizes):
            self.encoders.append(ConvLayer(input_size, output_size, kernel_size=5,
                                           stride=2, padding=2, norm=self.norm, running_stats=self.running_stats))
...
class UNetRecurrent(BaseUNet):
    ...
    def __init__(self, num_input_channels, num_output_channels=1, skip_type='sum',
                 ...
                 num_residual_blocks=2, norm=None, running_stats = True, use_upsample_conv=True):
        super(UNetRecurrent, self).__init__(num_input_channels, num_output_channels, skip_type, activation,
                                            num_encoders, base_num_channels, num_residual_blocks, norm, running_stats,
                                            use_upsample_conv)
        ...
        for input_size, output_size in zip(self.encoder_input_sizes, self.encoder_output_sizes):
            self.encoders.append(RecurrentConvLayer(input_size, output_size,
                                                    ...
                                                    norm=self.norm, running_stats=self.running_stats))
    ...
    def forward(self, x, prev_states):
        ...
        states = []
        for i, encoder in enumerate(self.encoders):
            # name_str = 'encoder_{}'.format(i)
            # with CudaTimer(device=torch.device('cuda:0'), timer_name=name_str):
        ...
        for i, resblock in enumerate(self.resblocks):
            # name_str = 'resblock_{}'.format(i)
            # with CudaTimer(device=torch.device('cuda:0'), timer_name=name_str):
            x = resblock(x)

        # decoder
        for i, decoder in enumerate(self.decoders):
            # name_str = 'decoder_{}'.format(i)
            # with CudaTimer(device=torch.device('cuda:0'), timer_name=name_str):
            x = decoder(self.apply_skip_connection(x, blocks[self.num_encoders - i - 1]))
