...
class ConvLayer(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, activation='relu', norm=None, running_stats=True):
        ...
        if norm == 'BN':
            self.norm_layer = nn.BatchNorm2d(out_channels, track_running_stats=running_stats)
        elif norm == 'IN':
            self.norm_layer = nn.InstanceNorm2d(out_channels, track_running_stats=running_stats)
...
class RecurrentConvLayer(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1, padding=0,
                 recurrent_block_type='convlstm', activation='relu', norm=None, running_stats=True):
        ...
        self.conv = ConvLayer(in_channels, out_channels, kernel_size, stride, padding, activation, norm, running_stats)
...
class ResidualBlock(nn.Module):
    def __init__(self, in_channels, out_channels, stride=1, downsample=None, norm=None, running_stats=True):
        super(ResidualBlock, self).__init__()
        bias = False if norm == 'BN' else True
        self.conv1 = nn.Conv2d(in_channels, out_channels, kernel_size=3, stride=stride, padding=1, bias=bias)
        self.norm = norm
        if norm == 'BN':
            self.bn1 = nn.BatchNorm2d(out_channels, track_running_stats=running_stats)
            self.bn2 = nn.BatchNorm2d(out_channels, track_running_stats=running_stats)
        elif norm == 'IN':
            self.bn1 = nn.InstanceNorm2d(out_channels, track_running_stats=running_stats)
            self.bn2 = nn.InstanceNorm2d(out_channels, track_running_stats=running_stats)
...
class ConvLSTM(nn.Module):
    ...
    def forward(self, input_, prev_state=None):
        ...
        stacked_inputs = torch.cat((input_, prev_hidden), 1) # stack input and hidden state
