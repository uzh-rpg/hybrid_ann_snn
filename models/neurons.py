...
class Mp_ParametricLIFNode(MpNode):
    def __init__(self, init_tau: float = 2.0, v_threshold: float = 1.,
                 v_reset: float = 0., surrogate_function: Callable = surrogate.Sigmoid(),
                 detach_reset: bool = False, shared_across_channels: bool = True, in_channels: int = 0):

        assert isinstance(init_tau, float) and init_tau > 1.
        super().__init__(v_threshold, v_reset, surrogate_function, detach_reset)
        
        self.shared_across_channels = shared_across_channels
        if self.shared_across_channels: # same membrane time constant accross channels of a layer
            init_w = - math.log(init_tau - 1.)
            self.w = nn.Parameter(torch.as_tensor(init_w))
        else: # channel-wise different membrane time constant 
            assert in_channels != 0
            init_w = - math.log(init_tau - 1.) * torch.ones(in_channels, 1, 1)
            self.w = nn.Parameter(init_w)

    def extra_repr(self):
        with torch.no_grad():
            tau = 1. / self.w.sigmoid()  # .sigmoid()
...
class CustomVinitLIFNode(BaseNode):
    ...
    def neuronal_charge(self, x: torch.Tensor, v_init: torch.Tensor):
            # note that self.v == 0. after reset, and self.v becomes a tensor after any forward
        if v_init != None: # If a v_init is passed in 
            self.v = v_init
    ...
    def forward(self, x: torch.Tensor, v_init: torch.Tensor):
        self.neuronal_charge(x, v_init)
        self.neuronal_fire()
        self.neuronal_reset()
        return self.spike, self.v

class CustomVinitMpLIFNode(MpNode):
    def __init__(self, tau: float = 2., v_threshold: float = 1.,
                 v_reset: float = 0., surrogate_function: Callable = surrogate.Sigmoid(),
                 detach_reset: bool = False):
